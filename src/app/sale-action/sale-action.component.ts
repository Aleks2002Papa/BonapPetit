...
@Component({
  selector: 'app-sale-action',
  standalone: false,
  templateUrl: './sale-action.component.html',
  styleUrl: './sale-action.component.css'
})
export class SaleActionComponent {
  myForm: FormGroup;
  ...
  ngOnInit(): void {
    this.saleId.set(this.route.snapshot.paramMap.get('id') ?? undefined);
    ...
    if (this.saleId() && (this.crudMode === CrudModeEnum.Edit || this.crudMode === CrudModeEnum.View)) {
      this.getSaleById();
    }
  ...
  onSubmit() {
    if (this.myForm.valid) {
      if (this.crudMode === CrudModeEnum.Add) {
        this.spinner.show();
        this.apiCallsService.createSale(this.myForm.value).subscribe(res => {
          if (!res) {
            this.spinner.hide();
      ...
      if (this.crudMode === CrudModeEnum.Edit) {
        this.spinner.show();
        this.apiCallsService.updateSale(this.myForm.controls['id'].value, this.myForm.getRawValue()).subscribe(res => {
         if (!res) {
            this.spinner.hide();
            this.toastr.error('Ndodhi nje gabim!');
            return;
  ...
  getSaleById() {
    return this.apiCallsService.getSaleById(this.saleId()).subscribe(res => {
      if (!res) return;
      this.formFill(res);
    })
  }
